...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        print(rout_data_time_list)
        for i in rout_data_time_list:
            user_query = NewTrip.objects.filter(rout_data_time=i, status_trip=False)
            print(user_query)
            for i in user_query:
                user_list.append(i.user)
            print(user_list)
        for i in rout_data_time_list:
            NewTrip.objects.filter(rout_data_time=i, status_trip=False).delete()
        for user in user_list:
            new_trip = TripsUser.objects.get(user__username=user)
            new_trip.quantity_default_trip += 1
            new_trip.save()
